...
/* ---------- chat stream ---------- */
function askStream(model, prompt, temperature = 0.7, images = []) {
...
/* ---------- generate stream ---------- */
function generateStream(model, prompt, temperature = 0.7, images = []) {
  const ev = new EventEmitter();
  const body = JSON.stringify({ model, prompt, images, temperature, stream: true });
